...
authors:
  - name: Kristoffer Ahlstrom-Vij
    address: k.ahlstrom-vij@bbk.ac.uk
abstract: |
...
## Incentives
The incentive received for participating across all conditions was £0.38, corresponding to an estimated £7.60 per hour, given that taking part in the study was expected to take about 3 minutes. The idea behind making the potential reward of £20 (£10 per question) in the treatment conditions substantially larger than the base incentives --- 52 times higher, to be exact --- was exactly to incentivise the type of truthful revelation and information discovery that, as we have seen, are taken by many to explain the accuracy of prediction markets.
...
The manipulation check disqualified 7 participants in the external resolution condition, and 89 in the self-resolution condition, leaving a final sample of 1108 participants, distributed across measured demographics as in Table \ref{tab:participant_table} in the Appendix. The table shows a fairly balanced demographic distribution across the conditions, which suggests that random allocation in recruitment was largely successful.
...
knitr::kable(two_tables_into_one,caption="Mean estimates and errors by condition (SD in paranthesis)",format="latex",
...
tosterplot_1 <- ggplot(plot_tosts[1:2,], aes(x=condition, y=difference, group=condition, color=condition)) + 
...
tosterplot_2 <- ggplot(plot_tosts[3:4,], aes(x=condition, y=difference, group=condition, color=condition)) + 
  geom_pointrange(aes(ymin=lwr, ymax=upr)) + # green
  geom_hline(yintercept=0, linetype="11", color = "grey") +
  geom_hline(yintercept=-bound, linetype="11", color = "black") +
  geom_hline(yintercept=bound, linetype="11", color = "black") +
...
If we look at any lack of overlap between the confidence intervals and the dashed vertical line at 0 in Fig. \ref{fig:equiv_test}, we see the outcome of a set of traditional null hypothesis tests. Specifically, we see that the only significant difference is between the self-resolving condition and the control for the urn task. (This was the aforementioned effect that was in the "wrong" direction, as far as our LOWER ERROR hypothesis was concerned.) The remaining differences are not significant. However, as we are performing an equivalency test, we now want to consider whether the confidence intervals clear the outer equivalency bounds at +/- 5 percentage points. In so far as they do, the mean errors for the relevant condition and the control are practically insignificant. Indeed, we see that, in the vaccination condition, which is arguably the most realistic estimation task of the two, the mean errors are practically equivalent all the way down to 2.5 percentage points. Consequently, we conclude that PRACTICAL EQUIVALENCY is consistent with the evidence.

# A third role for accuracy inventives?

In Section \ref{introduction}, we noted that the literature generally points to incentives on prediction markets performing the dual role of truthful revelation and information discovery. The evidence provided for PRACTICAL EQUIVALENCY in the previous section calls into question whether incentives really offer such a mechanism --- if it did, then we would have expected to see a significant accuracy effect from the two incentivisation conditions, which we did not.

However, there is another manner in which incentives might be able to increase accuracy on prediction markets, and that is by drawing in knowledgeable people, motivated by the incentives, who might not have joined the markets in the absence of such incentives. One way to spell this out is with reference work suggesting that a typical prediction market works by having a fairly large number of uninformed traders provide liquidity, while a small number of marginal traders take positions opposite to those of the uninformed, and in virtue of their trading volume also end up driving the market [@FORSYTHE199983; see also @OlivenRietz2004]. Indeed, these marginal traders tend to trade higher-than-average sums and to be active on the market on a higher-than-average number of days. They also earn higher-than-average returns, and are less prone to cognitive biases (such as assuming that their views are shared by others, or being inappropriately optimistic when interpreting evidence that supports their preferred outcomes).

Tying this back to accuracy incentives, the idea would be that incentives attract exactly such marginal traders, who in turn will drive accuracy. Notice, however, that suggesting that marginal traders _in actual fact_ drive accuracy does not go to show that they, counterfactually, would _not_ do so in the absence of incentives. To investigate that counterfactual, we would ideally be able to leverage experimental evidence showing that, in the presence of incentives, more accurate participants are attracted, compared to in the absence of incentives. But that, of course, is exactly the type of experiment we have reported on in this paper. When recruiting for this study, we recruited for the incentivised conditions by noting the extra, performance dependent incentive. We did not do that in the control condition. (See Section \ref{recruitment-texts} in the Appendix for the full recruitment texts.) This provides a between-subjects test of the hypothesis under consideration that, if accurate respondents turn up in greater number in the presence of incentives, then we should expect to see more of these in the incentivised condition. But again, the evidence provided for PRACTICAL EQUIVALENCY suggests that this did not happen.

# Discussion

The results of this study call into question the idea that prediction markets tend to generate accurate outputs on account of incentivising accuracy, whether by motivating people to provide more honest estimates, collect further information, or --- as discussed in the previous section --- attracting accurate traders to at all join the market. As noted in \ref{experimental-conditions-and-incentives}, participants took significantly longer to complete the survey in both incentivised conditions, compared to the control group, suggesting greater effort and potentially also attempts on the part of participants to locate relevant information (at least in relation to the vaccination task). However, in neither condition was the level of accuracy significantly different from that in the control condition. This offers evidence that incentivising for accuracy does not make a practically significant difference to estimation errors in a set of estimation tasks mirroring the type of incentive structures typically found on such markets. 
...
## Data
The data and code underlying the present paper can be accessed at [https://github.com/ahlstromvij/resolution](https://github.com/ahlstromvij/resolution).
